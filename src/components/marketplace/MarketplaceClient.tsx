...
const getDistanceRange = (distanceFilter: string | null): [number, number] => {
  switch (distanceFilter) {
    case '5': return [4, 6]
    case '10': return [9, 11]
    case '21': return [20, 22]
    case '42': return [41, 43]
    case '80': return [80, Infinity]
    case 'tri-s': return [25, 30] // Sprint distance total
    case 'tri-m': return [50, 55] // Olympic distance total
    case 'tri-l': return [110, 115] // Half Ironman total
    ...
    <div className="flex flex-col space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filteredAndSortedBibs.map(bib => (
          <CardMarket key={bib.id} bibSale={bib} />
        ))}
      </div>
    </div>
  )
