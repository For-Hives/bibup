import { pb } from '@/lib/pocketbase'; // Assuming PocketBase client is initialized here
...
export async function createUser(userData: CreateUserDTO): Promise<User | null> {
  try {
    const newUserRecord = {
 ...
 */
export async function fetchUserByClerkId(clerkId: string): Promise<User | null> {
  if (!clerkId) {
 ...
 */
export async function updateUserBalance(clerkUserId: string, amountToAdd: number): Promise<User | null> {
  if (!clerkUserId) {
  ...
  try {
    ...
    console.log(`User ${clerkUserId} balance updated from ${currentBalance} to ${newBalance}.`);
