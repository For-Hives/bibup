...
export type CreateBibData = Partial<Omit<Bib, 'id' | 'sellerUserId' | 'status' | 'buyerUserId' | 'privateListingToken' | 'eventId'>> & {
...
export async function createBib(
  ...
  if (bibData.isNotListedEvent) {
    if (!bibData.unlistedEventName || !bibData.unlistedEventDate || !bibData.unlistedEventLocation) {
      console.error("For unlisted events, event name, date, and location are required.");
      return null;
    }
  ...
  try {
    const dataToCreate: Omit<Bib, 'id'> & {
      ...
      unlistedEventName: bibData.isNotListedEvent ? bibData.unlistedEventName : undefined,
      unlistedEventDate: bibData.isNotListedEvent ? bibData.unlistedEventDate : undefined, // Ensure correct date format if model expects Date
      unlistedEventLocation: bibData.isNotListedEvent ? bibData.unlistedEventLocation : undefined,
    };
 ...
 */
async function initiateOrganizerNotification(bibId: string, newRunnerUserId: string, eventId?: string) {
  ...
  console.log(`INFO: Bib transfer notification process initiated for Bib ID: ${bibId}, New Runner (Buyer ID): ${newRunnerUserId}, Event ID: ${eventId || 'N/A'}. This would typically trigger an email or API call to the event organizer to update their participant list.`);
...
export async function updateBibStatusByAdmin(
  ...
  try {
    ...
    console.log(`ADMIN ACTION: Bib ${bibId} status changed to ${newStatus}. Notes: ${adminNotes || 'N/A'}`);
...
import { createTransaction, PLATFORM_FEE_PERCENTAGE } from "./transaction.services";
import { updateUserBalance, fetchUserByClerkId } from "./user.services";
...
export async function processBibSale(
  ...
  try {
    ...
    if (bib.status !== 'listed_public') {
      ...
      return { success: false, error: `Bib is not available for sale. Current status: ${bib.status}.` };
    }
    ...
    if (!sellerBalanceUpdated) {
      ...
      await pb.collection('transactions').update(transaction.id, { status: 'failed', notes: 'Seller balance update failed after fund capture.' });
      console.error(`CRITICAL: Failed to update seller ${bib.sellerUserId} balance for transaction ${transaction.id}.`);
      return { success: false, error: "Failed to update seller's balance. Transaction recorded but needs attention." };
    }
    ...
    console.log(`Bib ${bibId} sold to ${buyerUserId}. Transaction ${transaction.id} completed. Seller ${bib.sellerUserId} received ${amountToSeller}.`);

    // 7. Initiate Organizer Notification (Conceptual)
    if (updatedBib) { // Ensure bib was successfully marked as sold
        await initiateOrganizerNotification(updatedBib.id, updatedBib.buyerUserId!, updatedBib.eventId);
    }
 ...
 */
export async function fetchBibByIdForSeller(bibId: string, sellerUserId: string): Promise<(Bib & { expand?: { eventId: Event } }) | null> {
...
export type UpdateBibData = Partial<Pick<Bib, 'price' | 'originalPrice' | 'size' | 'gender'>>;
